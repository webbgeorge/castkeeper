...
func NewDEKService(cfg config.Config) (*dekService, error) {
	kekAEAD, err := getKeyEncryptionKeyAEAD(cfg)
	if err != nil {
		return nil, err
	}
...
func getKeyEncryptionKeyAEAD(cfg config.Config) (tink.AEAD, error) {
	// TODO support other drivers
	return DeriveAEADFromSecret(
...
func (s *dekService) ReEncryptKeys(newCfg config.Config) error {
	handle, err := s.loadOrCreate()
	if err != nil {
		return err
	}

	newKEKAEAD, err := getKeyEncryptionKeyAEAD(newCfg)
	if err != nil {
		return err
	}
...
func (s *dekService) aead() (tink.AEAD, error) {
	handle, err := s.loadOrCreate()
	if err != nil {
		return nil, err
	}
